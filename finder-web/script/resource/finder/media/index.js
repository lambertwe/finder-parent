...
AudioPlayer.prototype.create = function() {
    ...
    parent.find("div.title span.close").click(function() {
        self.pause();
        self.setPlayList(new PlayList());
        self.close();
        TaskManager.remove(self);
    });

    parent.find("div.play-list").each(function() {
    ...
    parent.find(".list-show").click(function() {
        var mode = parent.attr("mode");

        if(mode != "min") {
            parent.attr("mode", "min");
            parent.css("height", "180px");
            parent.find("div.play-list").hide();
        }
        else {
            parent.attr("mode", "max");
            parent.css("height", "540px");
            parent.find("div.play-list").show();
        }
    });
    ...
    parent.find(".volume .slider").click(function(event) {
        var src = jQuery(this);
        var x = event.offsetX;
        var width = src.width();
...
AudioPlayer.prototype.getContent = function() {
    ...
    buffer[buffer.length] = "<div class=\"icon repeat\" mode=\"1\" title=\"列表循环\"></div>";
    buffer[buffer.length] = "<div class=\"icon shuffle\" mode=\"2\" title=\"随机播放\"></div>";
    buffer[buffer.length] = "<div class=\"icon repeat once\" mode=\"3\" title=\"单曲循环\"></div>";
    buffer[buffer.length] = "<div class=\"icon list-show\"></div>";
    buffer[buffer.length] = "</div>";
    buffer[buffer.length] = "</div>";
    buffer[buffer.length] = "</div>";
...
AudioPlayer.prototype.add = function(item, play) {
    var self = this;
    var container = this.getContainer();
    var parent = jQuery(container);
    var ul = jQuery(container).find("div.play-list ul");
    ...
    var track = this.playList.size();

    this.playList.add(item);
    ul.append("<li track=\"" + track + "\" title=\"" + item.name + "\"><span class=\"title\">" + item.name + "</span><span class=\"remove\">删除</span></li>");

    parent.find("div.play-list ul li span.remove").unbind();
...
AudioPlayer.prototype.play = function(track) {
    ...
    jQuery(container).find(".tag div.caption").html(item.name);
    jQuery(container).find(".playback").addClass("playing");
...
AudioPlayer.prototype.doplay = function() {
    if(this.mode == 1) {
        this.next();
    }
    else if(this.mode == 2) {
...
AudioPlayer.prototype.prev = function() {
    console.log("prev - this.mode: " + this.mode + " - track: " + this.playList.getTrack());
...
AudioPlayer.prototype.visualize = function() {
    ...
    this.timer = setInterval(function() {
        ...
        if(audio.currentTime != self.currentTime) {
            self.lastTime = 0;
            ...
            if(self.lastTime >= 3000) {
                var url = audio.src;
                var currentTime = audio.currentTime + 3;
                console.log("audio.error: " + audio.currentTime + "/" + audio.duration + ", " + url);
...
VideoPlayer.prototype.schedule = function() {
    var self = this;
...
VideoPlayer.prototype.play = function(track) {
    ...
    this.playList.setTrack(track);
    jQuery(container).find(".cover").html("<img src=\"" +item.cover + "\" alt=\"" + (item.album || "") + "\">");
    jQuery(container).find(".tag").html("<strong>" + item.name + "</strong>");
    jQuery(container).find(".playback").addClass("playing");
    jQuery(container).find(".play-list li").removeClass("playing").eq(track).addClass("playing");
